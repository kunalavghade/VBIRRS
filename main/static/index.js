...
const tempLoad = () => {
	...
	tags.forEach((tag) => {
		$(".tags").append(`<button class='tag' data-tag="${tag}" >${tag}</button>`);
	});
...
$(function () {
	const showAlert = (type, msg) => {
		...
		alertTag.text(msg);
		alertTag.addClass(type + " active");
		setTimeout(() => {
			alertTag.removeClass(type + " active");
		}, 5000);
	};
	...
	$("#cambtn").on("click", function () {
		navigator.mediaDevices
			...
			.catch((err) => {
				showAlert("err", "Camera couldn't be accessed!");
